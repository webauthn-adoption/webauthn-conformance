...
import { generateBasicAttestationResponse } from "../../../../authenticators/generateBasicAttestationResponse.ts";
...
async function attestationResponseResp1P1Test(): Promise<void> {
  ...
  const opts1: PublicKeyCredentialCreationOptionsJSON = await httpClient
    .postAttestationOptions(opts).then((resp) => resp.json());

  generateBasicAttestationResponse(opts1);
